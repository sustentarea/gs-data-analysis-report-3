...
- The Brazilian Institute of Geography and Statistics ([IBGE](https://www.ibge.gov.br/)) Automatic Retrieval System ([SIDRA](https://sidra.ibge.gov.br/)), for data on GDP per capita [@ibgen; @ibgel].
...
data |>
  ...
  tabset_panel_gam(
    rhs_formula = "s(spei_12m) + s(gdp_per_capita) + s(year, bs = 're')",
    type = model_id
  )
```
...
data |>
  tabset_panel_gam(
    rhs_formula = "s(spei_12m) + s(gdp_per_capita) + year",
    type = model_id
  )
```
