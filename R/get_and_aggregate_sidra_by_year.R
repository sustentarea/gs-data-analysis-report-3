...
#'   )
#' }
get_and_aggregate_sidra_by_year <- function(
    years,
    api_start,
    api_end,
    year_col = "Ano"
  ...
  prettycheck:::assert_string(api_start)
  prettycheck:::assert_string(api_start)
  prettycheck:::assert_string(year_col)
  ...
  for (i in years) {
    ...
    if (inherits(data_i, "try-error")) {
      error_message <- attributes(data_i)$condition$message
